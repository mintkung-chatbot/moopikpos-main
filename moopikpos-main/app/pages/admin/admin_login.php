...
if (admin_is_logged_in()) {
    header('Location: ' . auth_url('admin_dashboard.php'));
    exit;
}
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        if (!$user || !$isPasswordValid || ($user['role'] ?? '') !== 'admin') {
            $errorMessage = 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง หรือไม่มีสิทธิ์เข้าใช้งานแอดมิน';
        } else {
...
<head>
    ...
    <style>
        body { font-family: 'Prompt', sans-serif; min-height: 100vh; background: linear-gradient(135deg, #f4f7ff 0%, #e5ecff 100%); display: flex; align-items: center; }
        ...
        <div class="card login-card">
            <div class="card-body p-4 p-md-5">
                <div class="d-flex align-items-center gap-3 mb-4">
                    <div class="login-badge"><i class="fa-solid fa-user-shield"></i></div>
                    <div>
                        <h1 class="h4 fw-bold mb-1">เข้าสู่ระบบแอดมิน</h1>
                        <p class="text-muted mb-0">กรอกบัญชีผู้จัดการเพื่อเข้าใช้งานหลังร้าน</p>
                    </div>
                </div>
                ...
                <form method="POST" action="<?php echo esc(auth_url('admin_login.php')); ?>">
                    <div class="mb-3">
                ...
                <a href="<?php echo esc(auth_url('index.php')); ?>" class="btn btn-link w-100 text-decoration-none mt-2">กลับไปหน้าเลือกโหมดระบบ</a>
            </div>
        </div>
    </div>
</body>
